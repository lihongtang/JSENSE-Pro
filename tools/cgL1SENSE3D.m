function [res] = cgL1SENSE3D(kData, x0, FT, MapOp, nIterCG, XOP, lambda, alpha,nIterSplit)
%
%[res] = cgL1SENSE3D(kData, x0, FT, MapOp, nIterCG, [ XOP, lambda, alpha,nIterSplit)
%
...
%     test = my_rss(MapOp*res);
% %  figure;montagesc(test(:,:,1));
    if lambda > 0
    ...
    figure(100), imshow3(abs(my_rss(MapOp * res)),[]), drawnow;

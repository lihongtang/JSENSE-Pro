...
switch(lower(method))
    case 'l2'
         ...
         maxit = 200;
         tol = 1e-6;
         recon_cg = pcg(AHA,AHb,[],[],[],[],recon_coe);
    ...
    case 'p+nn'
        ...
        brainMaskRef_trust = repmat(brainMaskRef,[1,1,1,size(B,2)]);
        
        param.E = subspace_model_error_mat(img,B,dim_spatial,undersampleMask);
        param.E_B = sub_subspace_mat(B,brainMaskRef_trust,dim_spatial); % multicoil model (b1: coil sensitivities)
        

        param.y = temp_undersample;
        AHA = @(x)param.E'*(param.E*x)+opt.pro_beta.*(x./sqrt(2.*opt.c_est_var));
        AHb = param.E'*param.y+opt.pro_beta.*(opt.c_est_mean./sqrt(2.*opt.c_est_var));
        % linear reconstruction
        if ~isfield(opt,'recon_cg')
            maxit = 200;
            recon_coe = param.E'*param.y;
            % recon_cg = mypcg(AHA,AHb,tol_al,maxit,[],[],recon_coe);
            recon_cg = pcg(AHA,AHb,[],[],[],[],recon_coe);
        ...
        while rel_err > tol_al
            ...
            %     AHb + .5*b*(param.E_B'*(U - Z)), tol_cg, maxit_cg, [], [], my_A );
            % toc

            % pcg_flag_check(flag,iter_pgc,relres);
            AHA = @(x)param.E'*(param.E*x)+opt.pro_beta.*(x./sqrt(opt.c_est_var))+.5*b*(param.E_B'*(param.E_B*x));
            AHb = param.E'*param.y+opt.pro_beta.*(opt.c_est_mean./sqrt(opt.c_est_var))+.5*b*(param.E_B'*(U - Z));
            maxit = 200;
            tol = 1e-6;
            % my_A = mypcg(AHA,AHb,tol,maxit,[],[],my_A);
            my_A = pcg(AHA,AHb,[],[],[],[],my_A);
        ...
        %     toc
            tic
            U = rank_trunc(reshape(temp_sen+Z,[],dim_spatial(4)), reg_para_nn/b, trunc_opt);
            U = reshape(U,dim_spatial);
            Z = Z + 1*(temp_sen-U);
            toc
    ...
    case 'p+nn2'
        if isfield(opt,'tol_cg')
            ...
            while rel_err > tol_al
                ...
                [my_A,flag,relres,iter_pgc]  = ...
                    pcg( @(x)(AHA(x) + .5*b*(param.E_B'*(param.E_B*x)) ), ...
                    AHb + .5*b*(param.E_B'*(U + Z/b)), tol_cg, maxit_cg, [], [], my_A );
                toc
            ...
            %     toc
                tic
                U = rank_trunc(reshape(temp_sen-Z/b,[],dim_spatial(4)), reg_para_nn/b, trunc_opt);
                U = reshape(U,dim_spatial);
                Z = Z + b*(U-temp_sen);
                toc

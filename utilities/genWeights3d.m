function w = genWeights3d(My,Mx,Mz,Nt,dirs,Iref,Iref_conf,percent_max_weight,display_weight_stats)
    
    ndirs  = length(dirs);
    c = zeros([My Mx Mz ndirs]);
    for m = 1:size(Iref,4)
        cur_Iref    = Iref(:,:,:,m);
    ...
    if display_weight_stats
        ...
        figure, montagesc(mean(c,4)), colormap(gray(256)), title('\bfMean Squared Finite Difference Values');
        figure, montagesc(mean(w,4)), colormap(gray(256)), title('\bfMean Weight Values');
    end

    % Replicate weights for all time points
    w = reshape(w, [My*Mx*Mz ndirs]);
    w = repmat(w,  [Nt 1]);
    w = reshape(w, [My Mx Mz Nt ndirs]);
